...
# 上级主管单位意见
    sponsor_comments = schema.Text(
        title=_(u"sponsor comments"), required=False)
# 民政局意见
    agent_comments = schema.Text(
        title=_(u"civil agent comments"),
        required=False)

# 上级主管单位审核日期
    sponsor_audit_date = schema.Text(
        title=_(u"sponsor audit date"), required=False)
# 民政局审核日期
    agent_audit_date = schema.Text(
        title=_(u"civil agent audit date"),
        required=False)
...
# 年检报告书
    report = NamedBlobFile(title=_(u"report"),
                           description=_(
                               u"Attach your anual report (word, etc)."),
                           required=True
                           )
# 上次工作流状态
