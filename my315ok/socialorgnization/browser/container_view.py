...
# title value is bytestr that encoded by utf-8
        for i in self.getMemberList():
            ...
            if num == 0:
                ...
                surveyid = IUserPreferredFileNameNormalizer(
                    self.request).normalize(newid)
...
#        memberbrains = catalog({'id':'shehuizuzhifengcai'})
        memberbrains = catalog(
            {'id': 'chachujieguogonggao', 'object_provides': IFolder.__identifier__})
#        import pdb
#        pdb.set_trace()
        top = memberbrains[0].getPath()
        allfolders = self.getFolders(top)
...
class favoritemoreb2(favoritemore):
    ...
    def render(self):
        ...
        nextstart = formstart + 10
        favorite_view = getMultiAdapter(
            (self.context, self.request), name=u"view")
        favoritenum = len(favorite_view.allitems())
...
class AnnualSurveyFolderView(OrgnizationsView):
    ...
    def allitems(self):
        ...
        mview = getMultiAdapter(
            (folder, self.request), name=u"orgnizations_survey_fullview")
...
class AdministrativeLicenceFolderView(AnnualSurveyFolderView):
    ...
    def allitems(self):
        ...
        folder = self.getOrganizationFolder()
        mview = getMultiAdapter(
            (folder, self.request), name=u"orgnizations_administrative_fullview")
        return mview.allitems()
