...
class Orgnizations_adminView(grok.View):
    ...
    def getOrgnizationFolder(self):
        ...
        topicfolder = self.catalog()(
            {'object_provides': IOrgnizationFolder.__identifier__})
        canManage = self.pm().checkPermission(
            permissions.AddPortalContent, self.context)
    ...
    def getMemberList(self):
        ...
        for brain in memberbrains:
            ...
            row['orgnization_passDate'] = brain.orgnization_passDate.strftime(
                '%Y-%m-%d')
    ...
    def getAnnualSurveyList(self):
        ...
        for i in braindata:
            ...
            <td class="result">%(annual_survey)s</td></tr>""" % dict(objurl=i.getURL(),
                                                                     title=i.Title,
                                                                     annual_survey=self.tranVoc(
                                                                         i.orgnization_annual_survey),
                                                                     year=i.orgnization_survey_year)
    ...
    def getAdministrativeLicenceList(self):
        ...
        for i in braindata:
            ...
            <td class="result">%(audit_date)s</td>
                            ...
                            title=i.Title,
                            audit_item=self.tranVoc(i.orgnization_audit_item),
                            audit_result=self.tranVoc(
                                i.orgnization_audit_result),
                            audit_date=i.orgnization_audit_date.strftime('%Y-%m-%d'))
            outhtml = "%s%s" % (outhtml, out)
        return outhtml
...
#        pdb.set_trace()
        favorite_view = getMultiAdapter(
            (self.context, self.request), name=u"orgnizations_survey_fullview")
    ...
    def outputList(self, braindata):
        ...
        for i in braindata:
            ...
            <td class="result">%(audit_result)s</td></tr>""" % dict(objurl=i.getURL(),
                                                                    title=i.Title,
                                                                    audit_item=self.tranVoc(
                                                                        i.orgnization_audit_item),
                                                                    audit_result=self.tranVoc(i.orgnization_audit_result))
            outhtml = "%s%s" % (outhtml, out)
        return outhtml
...
class AdministMore(grok.View):
    ...
    def render(self):
        ...
        nextstart = formstart + 10
        favorite_view = getMultiAdapter(
            (self.context, self.request), name=u"orgnizations_administrative_fullview")
        favoritenum = len(favorite_view.allitems())
...
class SiteRootAllOrgnizationListingView(Orgnizations_adminView):
    ...
    def buildAjaxViewName(self):
        ...
        if ISiteRoot.providedBy(context):
            return "oajaxsearch"
        elif IYuhuquOrgnizationFolder.providedBy(context):
            return "yuhuqusearch"
        elif IYuetangquOrgnizationFolder.providedBy(context):
            return "yuetangqusearch"
        elif IXiangxiangshiOrgnizationFolder.providedBy(context):
            return "xiangxiangshisearch"
        elif IXiangtanxianOrgnizationFolder.providedBy(context):
            return "xiangtanxiansearch"
        elif IShaoshanshiOrgnizationFolder.providedBy(context):
            return "shaoshanshisearch"
        else:
            return "xiangtanshisearch"
...
#            return    { "query": [start,],"range": "min" }
    ...
    def render(self):
        ...
        searchview = getMultiAdapter(
            (self.context, self.request), name=u"allorgnization_listings")
...
#        origquery['b_start'] = start
        ...
        if typekey != 0:
            origquery['orgnization_orgnizationType'] = searchview.getType(
                typekey)
...
# avoid autoform functionality
    ...
    @property
    def fields(self):
        return field.Fields(IOrgnization_annual_survey).select(
            'sponsor_comments', 'agent_comments', 'annual_survey')
