...
# checkPermission function must be use Title style permission
        canbe = self.pm().checkPermission(
            "my315ok.socialorgnization:Review anual report", self.context)
...
# checkPermission function must be use Title style permission
        canbe = self.pm().checkPermission(
            "my315ok.socialorgnization:Review lastly anual report", self.context)
...
# checkPermission function must be use Title style permission
        canbe = self.pm().checkPermission(
            "my315ok.socialorgnization:Add anual report", self.context)
...
# load viewlet
    def __getitem__(self, name):
        ...
        if viewlet is None:
            ...
            raise RuntimeError(
                "Viewlet does not exist by name %s for the active theme " %
                name)
        viewlet.update()
    ...
    def getViewletByName(self, name):
        ...
        for v in views:
            if v.provided == IViewlet:
                if v.name == name:
                    # if str(v.required[1]) == '<InterfaceClass
    ...
    def setUpViewletByName(self, name):
        ...
        try:
            ...
            raise RuntimeError(
                "Unable to initialize viewlet %s. Factory method %s call failed." %
                name)
        return viewlet
